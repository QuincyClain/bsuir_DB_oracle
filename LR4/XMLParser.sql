...
CREATE TABLE students (
  ...
  name VARCHAR2(50),
  groups_id NUMBER,
  CONSTRAINT fk_students_groups FOREIGN KEY (groups_id) REFERENCES groups(id)
);
...
BEGIN 
 ...
 <Join> 
 <Type>LEFT JOIN</Type> 
 <Condition>groups.id = students.groups_id</Condition> 
 </Join> 
 </Joins> 
 <Where> 
 <Conditions> 
...
ALTER TABLE groups ADD c_val NUMBER;

SELECT * FROM groups;

UPDATE groups SET c_val = 20 WHERE name = 'Group B'; 

INSERT INTO groups (id, name, c_val) VALUES (1, 'Group A', 10);
INSERT INTO groups (id, name, c_val) VALUES (2, 'Group B', 20);
...
INSERT INTO persons (id, name) VALUES (2, 'Jane');
...
CREATE TABLE other_table (col_2 VARCHAR(100) PRIMARY KEY);
...
BEGIN
    ...
    exe := 'CREATE TABLE CHEL.mytable ( col_1 NUMBER,  col_2 VARCHAR(100) NOT NULL, CONSTRAINT mytable_pk PRIMARY 
            KEY (col_1), CONSTRAINT mytable_other_table_fk Foreign Key(col_2) REFERENCES other_table(col_2))';
    EXECUTE IMMEDIATE exe;
END;

DROP TABLE CHEL.mytable;
